...
func TestGetOnlineClientsUnionDedupes(t *testing.T) {
	p := newOnlineTestProcess()
	p.RefreshLegacyXrayOnline([]string{"user1"}, nil, 1000, 20000)
	p.SetNodeOnlineTree(1, map[string][]string{"guid-a": {"user1", "user2"}})

	assertSameSet(t, "union", p.GetOnlineClients(), []string{"user1", "user2"})
}

// TestRefreshLegacyXrayOnlineGraceWindow checks the in-memory local set honours the
...
func TestRefreshLegacyXrayOnlineGraceWindow(t *testing.T) {
	p := newOnlineTestProcess()
	const grace = 20000

	p.RefreshLegacyXrayOnline([]string{"user1"}, nil, 1000, grace)
	if got := p.GetLocalOnlineClients(); !slices.Contains(got, "user1") {
		t.Fatalf("user1 should be online right after activity, got %v", got)
	}

	p.RefreshLegacyXrayOnline([]string{"user2"}, nil, 11000, grace)
	...
	p.RefreshLegacyXrayOnline(nil, nil, 22000, grace)
	got = p.GetLocalOnlineClients()
...
func TestGetLocalActiveInboundsTracksGraceWindow(t *testing.T) {
	...
	p.RefreshLegacyXrayOnline([]string{"alice"}, []string{"inbound-a"}, 1000, grace)
	assertSameSet(t, "active after first poll", p.GetLocalActiveInbounds(), []string{"inbound-a"})

	p.RefreshLegacyXrayOnline([]string{"alice"}, []string{"inbound-b"}, 11000, grace)
	...
	p.RefreshLegacyXrayOnline(nil, nil, 40000, grace)
